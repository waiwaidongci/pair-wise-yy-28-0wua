...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if len(parts)==4 and parts[:2]==["api","productions"] and parts[3]=="board": return self._json(200,self.db.production_board(int(parts[2])))
    ...
    def do_POST(self):
        ...
        try:
            ...
            if len(parts)==4 and parts[:2]==["api","productions"] and parts[3]=="precheck": return self._json(200,{"ok":True,**self.db.run_precheck(int(parts[2]))})
            if len(parts)==4 and parts[:2]==["api","productions"] and parts[3]=="shoot-days": return self._json(201,{"ok":True,"id":self.db.add_shoot_day(int(parts[2]),str(b.get("label","")),str(b.get("shoot_date","")),int(b.get("user_id",0)))})
            if len(parts)==4 and parts[:2]==["api","shoot-days"] and parts[3]=="scenes": return self._json(201,{"ok":True,"id":self.db.schedule_scene(int(parts[2]),int(b.get("scene_id",0)),int(b.get("queue_order",0)),int(b.get("user_id",0)))})
            if len(parts)==4 and parts[:2]==["api","shoot-days"] and parts[3]=="unschedule": self.db.unschedule_scene(int(parts[2]),int(b.get("scene_id",0)),int(b.get("user_id",0))); return self._json(200,{"ok":True})
            if len(parts)==4 and parts[:2]==["api","shoot-days"] and parts[3]=="release": return self._json(200,{"ok":True,**self.db.set_shoot_day_release(int(parts[2]),bool(b.get("release",False)),int(b.get("user_id",0)),str(b.get("note","")))})
